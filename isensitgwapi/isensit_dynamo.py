...
#    	self.table_save = self.dynamodb.Table(total_table_name)
	...
	currenttime = datetime.datetime.strptime(created_at,'%Y-%m-%d %H:%M:%S')-datetime.timedelta(hours=7)
    	try:
...
#        epoch_time = Decimal(time.time()).quantize(Decimal("0"))
    ...
    def insert_user_data2(self, deviceID, deviceInfoDict, deviceValueDict, created_at, shift):
#        epoch_time = Decimal(time.time()).quantize(Decimal("0"))
        upload_at = str(datetime.datetime.now())
        currenttime = datetime.datetime.strptime(created_at,'%Y-%m-%d %H:%M:%S')-datetime.timedelta(hours=7) #.%f')
        print ("------------------------currenttimt ", currenttime)
        Item_person = {
                'deviceID': deviceID,
                'gatewayID':self.gatewayID,
